...
Deno.test("PollingConcept: Operational Principle fulfillment", async () => {
  ...
  await pollingConcept.addOption({ actingUser: userAlice, poll: pollId, option: optionA });
  await pollingConcept.addOption({ actingUser: userAlice, poll: pollId, option: optionB });
  await pollingConcept.addOption({ actingUser: userAlice, poll: pollId, option: optionC });

  // 3. Alice adds Bob and Carol
  await pollingConcept.addUser({ actingUser: userAlice, poll: pollId, userToAdd: userBob });
  await pollingConcept.addUser({ actingUser: userAlice, poll: pollId, userToAdd: userCarol });

  // 4. Voting
  await pollingConcept.addVote({ user: userAlice, option: optionA, poll: pollId });
  await pollingConcept.addVote({ user: userBob, option: optionA, poll: pollId });
  await pollingConcept.addVote({ user: userCarol, option: optionB, poll: pollId });
...
Deno.test("PollingConcept: Adding duplicate option fails", async () => {
  ...
  await pollingConcept.addOption({ actingUser: userAlice, poll, option: optionA });
  const dup = await pollingConcept.addOption({ actingUser: userAlice, poll, option: optionA });

  assertExists((dup as { error: string }).error);
  assertEquals((dup as { error: string }).error, "Option already exists in this poll.");
...
Deno.test("PollingConcept: Add option after close fails", async () => {
  ...
  const res = await pollingConcept.addOption({ actingUser: userAlice, poll, option: optionA });
  assertEquals((res as { error: string }).error, "Cannot add option to a closed poll.");
...
Deno.test("PollingConcept: Remove option after close fails", async () => {
  ...
  const res = await pollingConcept.removeOption({ actingUser: userAlice, poll, option: optionA });
  assertEquals((res as { error: string }).error, "Cannot remove option from a closed poll.");
...
Deno.test("PollingConcept: Remove option also removes votes", async () => {
  ...
  await pollingConcept.addOption({ actingUser: userAlice, poll, option: optionA });
  await pollingConcept.addUser({ actingUser: userAlice, poll, userToAdd: userBob });
  ...
  await pollingConcept.removeOption({ actingUser: userAlice, poll, option: optionA });
...
Deno.test("PollingConcept: Add existing user fails", async () => {
  ...
  const res = await pollingConcept.addUser({ actingUser: userAlice, poll, userToAdd: userAlice });
  assertEquals((res as { error: string }).error, "User already participating in this poll.");
...
Deno.test("PollingConcept: Removing poll creator fails", async () => {
  ...
  assertEquals((res as { error: string }).error, "Cannot remove the poll creator.");
...
Deno.test("PollingConcept: Non-participant vote fails", async () => {
  ...
  const res = await pollingConcept.addVote({ user: userBob, option: optionA, poll });
  assertEquals((res as { error: string }).error, "User is not a participant in this poll.");
...
Deno.test("PollingConcept: Add vote after close fails", async () => {
  ...
  const res = await pollingConcept.addVote({ user: userBob, option: optionA, poll });
  assertEquals((res as { error: string }).error, "Cannot vote on a closed poll.");
...
Deno.test("PollingConcept: Update vote after close fails", async () => {
  ...
  await pollingConcept.addOption({ actingUser: userAlice, poll, option: optionA });
  await pollingConcept.addOption({ actingUser: userAlice, poll, option: optionB });
  await pollingConcept.addUser({ actingUser: userAlice, poll, userToAdd: userBob });
  await pollingConcept.addVote({ user: userBob, option: optionA, poll });
  await pollingConcept.close({ actingUser: userAlice, poll });

  const res = await pollingConcept.updateVote({ user: userBob, newOption: optionB, poll });
  assertEquals((res as { error: string }).error, "Cannot update vote on a closed poll.");
